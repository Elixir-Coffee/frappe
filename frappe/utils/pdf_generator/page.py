...
def resolve_intercepted_public_path(clean_path: str) -> tuple[str, bool]:
	...
	Elixir-Coffee/framework#55 (fork patch) — the non-asset branch previously required
	the path under ``public/files`` (native Frappe uploads). Apps that serve
	public files from another sub-tree — notably ``dfp_external_storage``, which
	rewrites external-storage File urls to ``/file/<name>/<filename>`` — fell
	outside that and were hard-blocked (``Fetch.failRequest``), aborting the print
	→ ``KeyError: 'result'`` in ``get_pdf_stream_id``. Widening to the whole
